...
function getActor(req, db) {
  ...
  if (!session || session.expiresAt < Date.now()) return db.user.email || "demo@businessbarber.local";
  const user = (db.users || []).find((item) => item.id === session.userId);
  return user.email || db.user.email || "demo@businessbarber.local";
}
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === "/api/me" && req.method === "GET") {
    return sendJson(res, 200, { user: publicUser(db.user), currentBarbershopId: db.currentBarbershopId });
  }
    ...
    const next = {
      ...
      users: Array.isArray(body.users)
         ?body.users.map((user) => {
            ...
            return { ...current, ...user, passwordHash: current.passwordHash || hashPassword("demo123") };
          })
        : db.users,
    };
    addAudit(next, "state.updated", getActor(req, db), { keys: Object.keys(body) });
    await writeDb(next);
